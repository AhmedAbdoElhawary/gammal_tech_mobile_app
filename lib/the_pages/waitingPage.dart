...
class _waitingPageState extends State<waitingPage> {
    ...
    return Scaffold(
      body: Container(
        ...
        child: Center(
          ...
          children: [
            if (widget.image != null) Column(
              children: [
                Image.asset(widget.image),
                buildText(widget.checkAnswer?"Correct Answer ! ":"Wrong Answer"),
              ],
            ),
            if (!widget.checkAccount) buildText("Sign in first"),
            SizedBox(height: 10,),
            Center(child:buildText("$_start"),
            ),
          ],
        )),
      ),
  ...
  Text buildText(String text) => Text(text,style: TextStyle(color: Colors.white,fontSize: 18));
